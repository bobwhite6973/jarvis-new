...
class Brain:
    ...
    def _detect_intent(self, query: str) -> str | None:
        ...
        if any(k in q for k in ["remember", "store", "save this"]):
            return "memory_store"
        if any(k in q for k in ["recall", "what did i", "what's my", "lookup"]):
        ...
        for provider in ordered:
            try:
                ...
                if len(ordered) > 1 and provider != ordered[0]:
                    answer = f"(via {PROVIDERS[provider]['label']} fallback)\n\n{answer}"
                return answer
            except Exception as e:
                last_error = str(e)
